  ...
  try {
    ...
    if (data.user == "student") {
      ...
      if (result2) {
        ...
        if (data.experience != "") workExperience.push(data.experience);
        if (data.achievement != "") achievements.push(data.achievement);
        ...
        if (result) {
          return resp.status(200).send({
            message: "Profile Data Updated in Database",
            success: true,
          });
        }
        return resp.status(500).send({
      ...
      const result2 = await recruiterModel.findOne({ userName: data.userName });
      if (result2) {
        if (!("profilePhoto" in data)) {
          const profilePhoto = files.profilePhoto[0];
          const imageKitPath = await imageKit.files.upload({
