...
const SignUp = () => {
  ...
  const signUpHandler = async (event) => {
    try {
      ...
      const response= await fetch("http://localhost:3200/signup-verification",{
        method:"POST",
      ...
      if(data.success){
         if(data.message == "User Account Already Exists in Database"){
            alert("Account Already Exists.Please Login!");
            navigate("/");
            return;
         }
         alert(data.message);
         navigate(`/otp-verification`,{state:{signUpData}});
         return;
      }
      ...
      <div className="h-fit w-72 lg:w-96 bg-white text-black rounded-t-xl flex justify-center py-7 lg:py-10">
        <div>
          <div className="text-center">
            <h1 className="font-bold text-md lg:text-lg">SignUp</h1>
          ...
          <div>
            ...
            <form action="" className="flex flex-wrap gap-8 justify-center">
              ...
              <button
                ...
                className="lg:w-80 w-64 p-2 bg-black rounded-lg flex justify-center text-sm lg:text-md items-center font-semibold text-white"
              >
                Continue
              </button>
            </form>
